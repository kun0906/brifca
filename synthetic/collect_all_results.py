""" Collect all the data generated by run_all.py if results.pkl already exists in each data_seed
    Otherwise, generate it from scratch.

    * Before run this script (almost the same as run_all.py; however main() is different),
    you should run run_all.py first.

    module load anaconda3/2021.11
    conda activate py3104_ifca
    PYTHONPATH='..' PYTHONUNBUFFERED=TRUE python3 collect_all_results.py > 'output/log.txt' 2>&1 &
...
def main(n=50):
    ...
    if is_debugging:
        ...
        cfg = {
            ...
            'alpha': [0.05],  # 0.05, percent of Byzantine machines
            ...
            'update_method': ['mean', 'median','trimmed_mean'], #gradient update methods for server, 'mean', 'median','trimmed_mean'
            'beta': [0.05],  # 0.05 trimmed means parameters
